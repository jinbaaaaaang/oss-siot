...
### 4. 텍스트 후처리 (`poem_text_processor.py`)
...
### 7. 프론트엔드 후처리 (`PoemGeneration.jsx`)
...
평균 BERTScore F1이 약 0.68인 건 시 생성 특성상 자연스럽습니다. 모델은 원본 시를 그대로 복사하지 않고 새로운 표현으로 재창조하기 때문에, 단어가 달라도 의미를 보존했다면 좋은 시입니다. 실제로는 일상글을 시로 변환하는 게 목표라서, 원본 시와 유사도가 낮아도 일상글의 의미를 잘 담았다면 충분히 의미 있는 결과입니다.
...
<div style="width: 100%; overflow-x: auto; min-height: 600px;">
...
| **scikit-learn** | 1.3.0+ | TF-IDF 키워드 추출, 분류 평가 지표 |
| **NumPy** | 1.24.0+ | 수치 연산 라이브러리 |
| **sentencepiece** | 0.1.99+ | 서브워드 토크나이저 |
| **datasets** | 2.14.0+ | Hugging Face 데이터셋 로딩 |
| **protobuf** | 4.0.0+ | 프로토콜 버퍼 |
| **bert-score** | - | BERTScore 평가 지표 (의미적 유사도 측정) |
| **matplotlib** | - | 데이터 시각화 (confusion matrix, 히스토그램) |
| **seaborn** | - | 통계 데이터 시각화 (히트맵) |
...
### 모델 학습 시 감정 분석 성능 측정의 어려움

모델 학습 및 평가 과정에서 감정 분석 성능을 정확하게 측정하기 어려웠습니다. 시가 감정을 간접적으로 표현하기 때문에 원본 텍스트의 감정과 생성된 시의 감정이 다르게 분류되는 경우가 많아, 감정 일치도를 평가 지표로 사용하기에는 신뢰성이 낮았습니다. 예를 들어, 원본 텍스트가 "기쁨"을 직접 표현해도 시는 추상적인 이미지로 전달하기 때문에 XNLI 분류기가 다르게 해석하여 오분류가 발생합니다. 이는 시의 본질적 특성 때문이지만, 모델의 감정 반영 능력을 객관적으로 평가하기에는 한계가 있었습니다.
...
현재 koGPT2 모델은 KPoEM 데이터셋의 시 원문만으로 학습되었습니다. 일상글을 시로 변환하는 패턴을 더 잘 학습시키려면, 일상글과 그에 대응하는 시를 쌍으로 묶은 데이터셋이 필요하지만, 현재는 그런 데이터셋이 존재하지 않아 학습이 불가능했습니다. 따라서 시 원문만으로 학습하거나 산문-시 변환 패턴을 간접적으로 학습하는 방식에 의존할 수밖에 없었습니다.
